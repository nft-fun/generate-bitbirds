...
The bird images are 24x24 arrays of variables, representing every pixel in the final image. There are four different array-matricies in the script; one for each type of bird. I'm not sure if array-matricies is valid terminology but thats how I picture it - basically one array for the bird, made of of 24 arrays - one for each line of pixels, and 24 variables in each line representing an individual pixel. If thats a confusing description, just forge ahead.

I've used variables with two letters for each type of pixel (e.g. outline `ol`, head color `hd`, beak color `bk`), so as to keep the 'matrix' of pixel variables easy to see and work with. 
